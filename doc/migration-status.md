...
| MIG-052 | 进行中 | 删除 legacy command/effect/mutation bridge   | MIG-051  | 已删除 `legacy-effects`、`mutation-bus` 与全部 `emitLegacyMutation` 调用；Menu 已移除全部 command 调用并直连 Pinia/领域事件；继续迁移 TownSquare 与 runtime bridge |
...
- Vitest 当前为 49 个测试文件、166 个测试；上次全局覆盖率为 statements 39.75%、branches 32.01%、functions 58.89%、lines 41.48%，本批新增 persistence、统一游戏事件与 Menu 直接 action 测试，但尚未重新生成全局覆盖报告。contracts/domain 已达到高覆盖率门禁，`socket.ts` 仍仅有 15.14% lines、7.85% branches；persistence 尚未设置专属门禁。
...
- `legacy-effects`、mutation bus 与 `emitLegacyMutation` 已删除；`Menu.vue` 已不再依赖 `legacy-commands`；TownSquare、runtime store 投影与 transport 仍待迁移；
