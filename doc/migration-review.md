...
本次检查运行于 Node `v25.3.0`、npm `11.9.0`。项目配置要求 Node `>=24.18.0 <25`，因此仍需在固定的 Node 24 LTS 环境重新运行一次完整检查，作为正式迁移和发布验收证据。
...
1. 使用 Node 24.18.0 重新执行 `npm ci` 和 `npm run check`；
...
1. Node 24 完整复验和迁移账本校准；
2. 会话层 characterization tests；
3. 拆分 session transport；
4. 重构 persistence；
5. 删除 legacy command/effect bridge；
6. 拆分 Menu、TownSquare、Player 和 Vote；
7. 扩展多客户端业务 E2E 与视觉矩阵；
8. 样式、资源和包体治理；
9. 服务端运维、兼容、canary 和回滚收尾。
...
- Node 24 LTS 下 `npm run check` 稳定通过；
